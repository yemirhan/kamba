...
export const itemRouter = router({
  ...
  getItemDetailsBySlug: protectedProcedure
  ...
  assignUserToItem: protectedProcedure
  ...
  unassignUserFromItem: protectedProcedure
    .input(z.object({ slug: z.string(), userId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      return await ctx.prisma.item.update({
        where: {
  ...
  updateItemStatus: protectedProcedure
    .input(z.object({ slug: z.string(), statusId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      return await ctx.prisma.item.update({
        where: {
          slug: input.slug,
